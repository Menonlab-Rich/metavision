        ...
        def __iter__(self):
            ...
            def structured_to_jnp(structured_array):
                ...
                arr = np.stack(
                    ...
                    [structured_array['x'], structured_array['y'], structured_array['p']],
                ...
                if len(current_evts_structured) > 0:
                    ...
                    if prev_evts_structured is not None:
                        ...
                        if pad_source_count > 0:
                            ...
                            padding = jnp.zeros((pad_source_count, 3), dtype=jnp.float32).at[:, -1].set(3)
                        ...
                        if pad_target_count > 0:
                            ...
                            padding = jnp.zeros((pad_target_count, 3), dtype=jnp.float32).at[:, -1].set(3)
            ...
            try:
                ...
                if target_points_np.shape[0] > 0:
                    ...
                    target_df = pd.DataFrame({
                        "frame": i,
                        "track_id": track_ids,
                        "x": target_points_np[:, 0],
                        "y": target_points_np[:, 1],
                        "p": np.array(padded_target_pts)[:num_target, 2],
